...
class ResultWindow(tk.Toplevel):
    def __init__(self, parent, calculated_data):
        ...
        self.text_label = tk.Label(self, text="Search the calories and macro nutrients for your desired foods to easier achieve your goals!", font=("Verdana", 12))
        self.text_label.pack(side=tk.LEFT, padx=10, pady=10)

        # Add the extra button on the side
        self.extra_button_image = Image.open("images\DiscoverNutrition.png")
        ...
        self.extra_button = tk.Button(self, image=self.extra_button_photo, command=self.perform_extra_action, borderwidth=0)
        self.extra_button.pack(side=tk.LEFT, padx=5, pady=10)

    # Create a frame to hold content
    def perform_extra_action(self):
        subprocess.call(["python", "Data\DiscoverNutrition.py"])
