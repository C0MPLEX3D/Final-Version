...
class CaloricIntakeCalculator:
    def __init__(self, master):#self means refering to the name tag
        self.master = master
        ...
        style.configure("TButton", borderwidth=0, padding=0, relief=tk.FLAT, background="#d0cdd1",
                        activebackground="#d0cdd1")

        self.radio_var = tk.StringVar()
        self.radio_var.set("male")

        self.radio_male = ttk.Radiobutton(self.content_frame, text="Male", variable=self.radio_var, value="male",
                                          style="TRadiobutton", command=self.update_button_state)
        self.radio_male.grid(row=1, column=1, sticky="W", pady=2)

        self.radio_female = ttk.Radiobutton(self.content_frame, text="Female", variable=self.radio_var,
                                            value="female", style="TRadiobutton", command=self.update_button_state)
        self.radio_female.grid(row=2, column=1, sticky="W", pady=2)

        self.label_weight = tk.Label(self.content_frame, text="Weight (kg):", font=("Verdana", 14), fg="Black",
                                     bg="#d0cdd1")
        self.label_weight.grid(row=3, column=0, sticky="E", pady=2)

        self.entry_weight = tk.Entry(self.content_frame, font=("Verdana", 12), width=20, bd=2, relief="solid",
                                     justify="center", highlightthickness=2, highlightbackground="#000000")
        self.entry_weight.grid(row=3, column=1, sticky="W", pady=20)

        self.label_height = tk.Label(self.content_frame, text="Height (cm):", font=("Verdana", 14), fg="Black",
                                     bg="#d0cdd1")
        self.label_height.grid(row=4, column=0, sticky="E", pady=20)

        self.entry_height = tk.Entry(self.content_frame, font=("Verdana", 12), width=20, bd=2, relief="solid",
                                     justify="center", highlightthickness=2, highlightbackground="#000000")
        ...
        self.entry_age = tk.Entry(self.content_frame, font=("Verdana", 12), width=20, bd=2, relief="solid",
                                  justify="center", highlightthickness=2, highlightbackground="#000000")
        self.entry_age.grid(row=5, column=1, sticky="W", pady=20)

        self.label_activity_level = tk.Label(self.content_frame, text="Activity Level:", font=("Verdana", 14),
                                              fg="black", bg="#d0cdd1")
    ...
    def calculate_caloric_intake(self):
        try:
            ...
            message = (
                f"Your Basal Metabolic Rate (BMR) is: {bmr:.2f} calories per day.\n"
                f"Your Total Daily Energy Expenditure (TDEE) is: {caloric_intake:.2f} calories per day.\n"
                f"Your Body Mass Index (BMI) is: {bmi:.2f}\n"
                "Activity Level Calories:\n"
                f"Sedentary: {bmr * 1.2:.2f} calories per day\n"
                f"Lightly Active: {bmr * 1.375:.2f} calories per day\n"
                f"Moderately Active: {bmr * 1.55:.2f} calories per day\n"
                f"Very Active: {bmr * 1.725:.2f} calories per day\n"
                f"Extra Active: {bmr * 1.9:.2f} calories per day"
            )
            messagebox.showinfo("Caloric Intake", message)
