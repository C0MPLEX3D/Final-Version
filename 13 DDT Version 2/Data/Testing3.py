...
class CaloricIntakeCalculator:
    ...
    def calculate_caloric_intake(self):
        try:
            ...
            self.label_calculated_bmi.config(text=f"{bmi:.2f}")

            # Create a results string to pass to ResultsPage
            results_text = (f"Your Basal Metabolic Rate (BMR) is: {bmr:.2f} calories per day.\n"
                            f"Your Total Daily Energy Expenditure (TDEE) is: {caloric_intake:.2f} calories per day.\n"
                            f"Your Body Mass Index (BMI) is: {bmi:.2f}")

            # Create an instance of ResultsPage and pass the results
            results_window = tk.Toplevel(self.master)
            results_page = ResultsPage(results_window, results_text)
