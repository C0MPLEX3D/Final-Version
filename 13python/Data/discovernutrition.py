"""
Discover Nutrition Application
This script creates a graphical user interface (GUI) application using the Tkinter library
to retrieve and display nutrition information for a given food item using an external API.
...
def get_nutrition():
    """
    Retrieve nutrition information for a food item from the API and display it in the GUI.
    """
    ...
    response = requests.get(api_url,
                        headers={'X-Api-Key': 'iq3i8dp7pwjmIbuCLYZ0mQ==DtkAcFwf90LgWAuS'},
                        timeout=10)  # Adjust the timeout value as needed (in seconds)
...
entry_query = tk.Entry(content_frame, width=30, bd=2, relief="solid",
...
result_text = tk.Text(content_frame, height=15, width=60, state=tk.DISABLED,
                      bd=2, relief="solid", highlightthickness=2, highlightbackground="#000000")
result_text.pack()
