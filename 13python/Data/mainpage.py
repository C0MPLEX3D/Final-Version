"""
This module contains the main page of the Caloric Intake Calculator application.
It provides a graphical user interface for calculating caloric intake based on various factors.
"""
...
class CaloricIntakeCalculator:
    ...
    def __init__(self, master):  # Constructor, receives the main window as 'master'
        ...
        style.configure("TButton", borderwidth=0, padding=0,
                        relief=tk.FLAT,
    ...
    def calculate_caloric_intake(self):
        ...
        """
        try:
            weight_text = self.entry_weight.get()
            height_text = self.entry_height.get()
            age_text = self.entry_age.get()
            if not weight_text.isdigit() or not height_text.isdigit() or not age_text.isdigit():
                raise ValueError("Please enter numeric values for weight, height, and age.")
            
            weight = float(weight_text)
            height = float(height_text)
            age = int(age_text)

            if len(weight_text) > 3:
                raise ValueError("Weight value cannot have more than 3 digits.")
            if len(height_text) > 3:
                raise ValueError("Height value cannot have more than 3 digits.")
            if len(age_text) > 3:
                raise ValueError("Age value cannot have more than 3 digits.")
